...
export function HabitsManager({ habits }: Props) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await createHabitAction({ label, icon, accent });
      if (!res.ok) {
            ...
            <li key={h.id} className={styles.habitItem}>
              ...
              <div className={styles.habitText}>
                ...
                <span className={styles.habitKind}>
                  {isWater ? "Water · auto" : "Yes / Half / No"}
                </span>
              </div>
              {isWater ? (
